...
- **Context menu** — Cut / Copy / Paste + Save Image As; spelling suggestions via `aspell` (falls back to English); links show **Open with [App]** and **Open in browser** (with the system default browser icon) when a routing target is known. Some apps (Teams, Office) suppress the browser context menu to show their own; **Ctrl+right-click** forces ours through wherever a `contextmenu` event still fires, and **Shift+F10** / the **Menu key** pops it via the keyboard in any app (intercepted in the main process, so it works even where the right-click is swallowed entirely — e.g. Word's editor). A plain right-click still goes to the app
