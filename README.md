...
**Remembered window position:** the extension also restores each widget to where you last left it. When a widget window closes, its frame (position + size) is saved to `~/.local/state/voltage/widget-geometry.json`, keyed by the app's `.desktop` id; on the next launch the window opens at that exact spot. This is again a Wayland necessity — a client cannot position its own window, only the compositor can. The restore is **monitor-aware**: if your display layout has changed so the saved frame would land off-screen (a monitor unplugged or rearranged), the saved position is ignored and GNOME places the window normally, so a widget can never disappear into a monitor that no longer exists.
