...
- **Zoom** — the optional **zoom** plugin adds `Ctrl+Scroll` zoom with an on-screen percentage panel and a context-menu submenu (in / out / reset); step size and bounds are set in its config dialog. Optionally (off by default) it **remembers the zoom level per page area** (first URL-path segment) and restores it on every navigation — for apps where one origin serves differently dense areas (e.g. document list vs. editor): just zoom once in each area and it sticks; unzoomed areas start at 100 %.
