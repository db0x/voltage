...
- **Widget mode** — the **widget** plugin renders an app as a frameless, rounded window (optional drop shadow, tint, hidden scrollbars). Combined with the [GNOME integration](#gnome-integration) it can be kept out of the dock entirely. To move the frameless window it adds a **top drag handle** (on by default): a centered control (responsive width — between half and 90% of the window, taking a larger share as the window narrows; styled like Voltage's link tooltip) that fades in when the cursor reaches the top-centre edge and fades out when it leaves, with a configure (gear) button on the far left that opens this app's settings in the Manager, window-control buttons on the right (About / minimize / maximize / close), and — for apps that load the zoom plugin — zoom controls (− / live level / +); hovering a button shows its label centred on the bar. It is our own overlay view rather than an app-region in the page because Chromium only honours `-webkit-app-region: drag` from a frame Voltage owns, not from the cross-origin iframes some apps render their toolbars in (e.g. Office documents) — so this works regardless of the app. The context-menu **Move** mode (also `F10`) remains as an alternative.
...
- **Custom CSS** — the optional **css-inject** plugin injects a per-app stylesheet into the wrapped page (set in its config dialog) to restyle or hide elements of the site. It offers a repeatable list of CSS variable → colour overrides plus a free-text CSS field appended verbatim to the injected stylesheet. It can also remove Chromium's keyboard focus ring (opt-in, off by default).
...
| `pluginConfig` | object | Per-plugin settings keyed by the plugin's webapps-relative path (e.g. `{ "plugins/widget/widget.js": { "radius": 20 } }`). Configurable plugins expose a gear button in the dialog and receive these as `api.config` at runtime. The **widget** plugin offers corner radius, drop shadow, resizable, hidden scrollbars, suppressed title bar, background tint, *Show in taskbar*, the *top drag strip* (on by default) and its *light/dark theme* (dark by default); the **zoom** plugin offers step size and min/max bounds; the **css-inject** plugin offers a repeatable list of CSS variable → colour overrides, a free-text custom-CSS field appended verbatim to the injected stylesheet, plus an opt-in toggle to remove Chromium's keyboard focus ring; the **docker-integration** plugin routes the app to a locally running Docker container instead of its online URL — pick a curated, bundled compose *stack* (the AppImage starts it via `docker compose up` on launch, auto-picks the next free port, and tears it down on exit), with optional advanced overrides for a fixed port or a custom compose file. It is greyed out when Docker/Compose is missing, and locks the app's URL field while selected. Changing a value requires a rebuild |
