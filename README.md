...
- **Zoom** — the optional **zoom** plugin adds `Ctrl+Scroll` zoom with an on-screen percentage panel and a context-menu submenu (in / out / reset); step size and bounds are set in its config dialog. Optionally (off by default) it **remembers the zoom level per page area** (first URL-path segment below the app's base URL, so reverse-proxy path prefixes don't collapse the areas) and restores it on every navigation — for apps where one origin serves differently dense areas (e.g. document list vs. editor): just zoom once in each area and it sticks; unzoomed areas start at 100 %.
...
- **Desktop notifications** — the optional **notifications** plugin gives an app working notifications under GNOME. Electron implements only *non-persistent* web notifications, so the service-worker path every PWA-style app uses (Teams, Outlook) is accepted, resolves, and is then dropped without ever reaching the desktop — and its `getNotifications()` counterpart never settles at all, stalling the app before it shows anything. The plugin re-points both at the main process, which raises a real notification, replaces same-tag toasts, and raises the app window when one is clicked (via the [GNOME integration](#gnome-integration)). Note that Electron ships no push service: notifications only arrive while the app is running. Details in the [plugin README](webapps/plugins/notifications/README.md).
...
**Remembered window position:** the extension also restores **every Voltage app** — widget or not — to where you last left it. This is independent of the **Show in taskbar** setting: as long as the extension is active, the feature applies to all Voltage AppImages. A window's frame (position + size) is recorded continuously — a couple of seconds after you stop moving or resizing it, and again when it closes — into `widget-geometry.json` **inside that app's own profile-data folder** (`~/.config/voltage/<profile>/`, or a custom `profileDir` if set); on the next launch the window opens at that exact spot. The extension finds the folder from the `X-Voltage-ProfileDir` line the Manager writes into every Voltage app's `.desktop` launcher. This is again a Wayland necessity — a client cannot position its own window, only the compositor can. The restore is **monitor-aware**: a saved frame is never reproduced verbatim when your display layout changed under it. A frame that would land off-screen (a monitor unplugged or rearranged) is slid back onto the nearest monitor at the same size, and a frame whose *size* is no longer usable is replaced by a sensible default, centered on the work area. That second case is what happens when a monitor or resolution switch leaves Mutter having shrunk a window to its minimum — without the repair that speck would be saved and faithfully restored, leaving a window that is almost impossible to find on the desktop. The same repair also runs on **already open** windows whenever the monitor layout changes (shortly after, once the compositor has settled), so a window shrunk by a display switch is rescued right away instead of at the next launch. Maximized, fullscreen and minimized windows are left alone.

**One position per monitor layout:** frames are remembered **per display arrangement**, not globally. The key is the set of monitors and their geometry (e.g. laptop panel alone vs. laptop + docked 4K on the right), so undocking and re-docking puts every Voltage window back where it was *on that setup*: unplug and the windows take their laptop-panel positions, plug the dock back in and they return to their dock positions — no relaunch needed, the switch happens shortly after the compositor settles. This is the part GNOME itself does not do for Wayland clients. An arrangement an app has never been open on falls back to its last known frame, repaired to fit the new screens, and gets its own entry from then on. Because recording is continuous, the positions of the layout you are *leaving* are already on disk when you unplug. Geometry files written by an older version of the extension keep working — their single stored frame becomes that fallback.
...
**Raising a window on request:** under Wayland a client may not bring itself to the front — the request has to come from the compositor. The extension exposes a small D-Bus method (`de.db0x.Voltage.ActivateApp`) that a running Voltage app calls to have its window activated, addressed by its `.desktop` launcher id. Today the **notifications** plugin uses it so clicking a notification actually brings the app forward. Without the extension the call fails immediately and the app falls back to Electron's own `focus()`, which under GNOME usually just marks the dash icon as ready. This needs extension **version 7 or newer**.
...
| `largeAssetCache` | boolean | Raise Chromium's disk-cache-size budget — Chromium appears to silently skip caching individual HTTP responses above roughly 100MB regardless of `Cache-Control`, so a WASM app streaming very large assets (e.g. hundreds of MB of audio/video) over HTTP would otherwise re-download them on every launch |
...
| `pluginConfig` | object | Per-plugin settings keyed by the plugin's webapps-relative path (e.g. `{ "plugins/widget/widget.js": { "radius": 20 } }`). Configurable plugins expose a gear button in the dialog and receive these as `api.config` at runtime. Options per plugin: [widget](webapps/plugins/widget/README.md) (frameless widget look & drag strip), [css-inject](webapps/plugins/css-inject/README.md) (per-app stylesheet), [docker-integration](webapps/plugins/docker-integration/README.md) (local container stacks, env/secrets), [relay](webapps/plugins/relay/README.md) (file sync to a self-hosted relay server), [robot](webapps/plugins/robot/README.md) (run a sequence of clicks / text entries on load, keep-alive against “Away” presence); the **zoom** plugin offers step size and min/max bounds. Changing a value requires a rebuild |
