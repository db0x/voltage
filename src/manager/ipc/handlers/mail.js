...
// In test mode, VOLTAGE_TEST_MAIL_HANDLER overrides the real xdg-mime query.
function getDefaultMailDesktop() {
  if (process.env.VOLTAGE_TEST) return process.env.VOLTAGE_TEST_MAIL_HANDLER || null
  try {
...
function registerMailHandlers() {
  ...
  ipcMain.handle('manager:set-mail-handler', (event, desktopName) => {
    if (process.env.VOLTAGE_TEST) return true
